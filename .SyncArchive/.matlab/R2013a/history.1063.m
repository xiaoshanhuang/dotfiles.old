...
%-- 12/9/13, 12:20 AM --%
eeglab
BCG = rSIM(ALLEEG(snBCG).data);
BCG = rPCA(ALLEEG(snBCG).data);
bcg = epoch(ALLEEG(snBCG).data, qrs, [-100 900]);
bcgSIM = rSIM(bcg);
bcgPCA = rPCA(bcg);
bcg = mean(bcg,3);
chan = 20; plot([bcg(chan,:); bcgSIM(chan,:); bcgPCA(chan,:)])
chan = 20; plot([bcg(chan,:); bcgSIM(chan,:); bcgPCA(chan,:)]')
eeglab redraw
vep = EEG.data;
vepSIM = rSIM(vep);
vepPCA = rPCA(vep);
vep = mean(vep,3);
chan = 20; plot([vep(chan,:); vepSIM(chan,:); vepPCA(chan,:)]')
vep = mean(vep,3);
vepSIM = rSIM(vep);
vep = EEG.data;
vepSIM = rSIM(vep);
...
vep = mean(vep,3);
chan = 20; plot([vep(chan,:); vepSIM(chan,:); vepPCA(chan,:)]')
chan = 20; plot([vep(chan,:); real(vepSIM(chan,:)); vepPCA(chan,:)]')
chan = 20; plot(mapstd[vep(chan,:); real(vepSIM(chan,:)); vepPCA(chan,:)]')
chan = 20; plot(zscore([vep(chan,:); real(vepSIM(chan,:)); vepPCA(chan,:)]'))
vep = EEG.data;
vepSIM = rSIM(vep);
vepPCA = rPCA(vep);
vep = mean(vep,3);
