%-- Unknown date --%
,3)'));r = diag(D);[r,I] = sort(r,'descend');V = V(:,I);plot(V(:,1:2)'*mean(epoch,3)')
...
help xcorr
[C, As, s] = resample_erp(epoch,chanlocs);
plot(xcorr(s',detrend((C'*x(:,:,iTrial))')))
[C, As, s] = resample_erp(epoch,chanlocs);
imagesc(t)
[C, As, s] = resample_erp(epoch,chanlocs);
imagesc(t)
...
[C, As, s] = resample_erp(epoch,chanlocs);
A(1)
lsqnonneg(reshape(pinv(C')*s,[nChan*nPoint,1]), reshape(x(:,:,iTrial),[nChan*nPoint,1]))
reshape(pinv(C')*s,[nChan*nPoint,1])\reshape(x(:,:,iTrial),[nChan*nPoint,1])
A(iTrial)
reshape(pinv(C')*s,[nChan*nPoint,1])\reshape(x(:,:,iTrial),[nChan*nPoint,1])
plot(xCorr(reshape(pinv(C')*s,[nChan*nPoint,1]), reshape(x(:,:,iTrial),[nChan*nPoint,1])))
plot(xcorr(reshape(pinv(C')*s,[nChan*nPoint,1]), reshape(x(:,:,iTrial),[nChan*nPoint,1])))
I
plot(t)
plot(t')
...
plot(t(1,:))
I
plot([s',detrend((C'*x(:,:,iTrial))')])
plot([s',detrend((C'*x(:,:,3))')])
plot([s',detrend((C'*x(:,:,4))')])
plot([s',detrend((C'*x(:,:,3))')])
plot([padarray(s',35),detrend((C'*x(:,:,3))')])
plot([padarray(s',35)])
plot([padarray(s',2)])
plot([padarray(s',-2)])
plot([padarray(s',2)])
...
help padarray
iChan>60 ? 1:2
padarray(times, 2, [], 'pre')
help padarray
padarray(times, 2, 0, 'pre')
padarray(times', 2, 0, 'pre')
padarray(times', 2, 0, 'post')
[C, As, s] = resample_erp(epoch,chanlocs);
t(1)
plot([sPad sEst])
plot([s sEst])
...
plot(s')
[C, As, s] = resample_erp(epoch,chanlocs);
[C1, As, s1] = resample_erp(epoch,chanlocs);
figure, topoplot(C1(:,1), chanlocs)
plot([s(1,:); s1(1,:)]')
[C1, As, s1] = resample_erp(epoch,chanlocs);
t
