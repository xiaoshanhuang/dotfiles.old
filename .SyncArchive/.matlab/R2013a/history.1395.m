...
EEGs{2,4}
snOBS = 3;
EEGs{1,4} = ALLEEG(snOBS);
clear all; close all; clc; jheapcl;
eeglab;
EEGs{:,:,1} = load('VEPs.mat');
EEGs{:,:,2} = load('VEPs.mat');
EEGs
EEGs{1,1,1}
subsEEG{1} = load('VEPs.mat');
clear EEGs
subsEEG{2} = load('VEPs.mat');
subsEEG{3} = load('VEPs.mat');
subsEEG{4} = load('VEPs.mat');
methods = {'AR', 'CCAR', 'CCAMW', 'OBS'};
subEEGs{1}
subsEEG{1}
subsEEG{1}{1,1}
subsEEG{1}{1}
...
chan = [9 10 20 31];
erp{2}
size(mean(erp{subject, method}(chan,:),1))
plot(erpAll)
snr
mean(snr,1)
snrAll
...
snr
snrAll
plot(subsEEG{1}.EEGs.times, erpAll)
plot(subsEEG{1}.EEGs{1,1}.times, erpAll)
plot(subsEEG{1}.EEGs{1,1}.times, erpAll')
size(subsEEG{1}.EEGs{1,1}.times)
plot(erpAll)
help boxplot
print -depsc fig.eps
